...
def get_product_price(soup):
    ...
    for span in price_spans:
        ...
        """try:
            ...
            print('Value Obtained for Price ')
            exit()"""
...
def get_product_rating(soup):
    ...
    product_rating_section = product_rating_div.find('i', attrs = {'class':'a-icon-star'})
    product_rating_span = product_rating_section.find("span")
    rating = product_rating_span.text.strip().split()
    #print(type(float(rating[0])))
    try:
        rating = product_rating_span.text.strip().split()
        #print(rating[0])
        return rating[0]
    except ValueError:
        print("Values Obtained For Rating Could Not Be Rating")
        exit()

def get_product_technical_details(soup):
    details = {}
    technical_details_section = soup.find('div', id = 'prodDetails')
    data_tables = technical_details_section.findAll('table', id = 'productDetails_techSpec_section_1')
    #print(data_tables)
    for table in data_tables:
        ...
        for row in table_rows:
            ...
            #details[row_value] = row_value
    return details


def extract_product_info(url, output):
    product_info = {}
    #print(f'Scraping_URL: {url}')
